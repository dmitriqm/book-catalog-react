...
const Catalog = () => {
  ...
  const [recommendedIndex, setRecommendedIndex] = useState(Math.floor(Math.random() * recommendations.length))
    ...
    <>
      <div className="w-full flex items-center flex-col mb-6">
        {recommendations[0] ? (
          <>
            ...
            >
              <BookCard book={recommendations[recommendedIndex < recommendations.length ? recommendedIndex : recommendedIndex - 1]} />
            </GroupWrapper>
            {recommendations.length > 1 ? (
              <button className="rounded-lg border px-4 py-2 bg-slate-200 hover:bg-slate-300" onClick={changeRecommendedIndex.bind(null)}>
                Показать другую
              </button>
            ) : null}
          </>
      ...
      {books[0] && (
        <div className="flex justify-center items-center mb-2">
          ...
          <button
            className={`border rounded-lg px-4 py-2 ${groupType === BookGroupType.Year ? 'bg-slate-300' : 'bg-slate-200'} hover:bg-slate-300 mr-1`}
            onClick={() => dispatch(changeGroupType(BookGroupType.Year))}
          >
            Году
          </button>
          <button
            className={`border rounded-lg px-4 py-2 ${groupType === BookGroupType.Author ? 'bg-slate-300' : 'bg-slate-200'} hover:bg-slate-300 mr-1`}
